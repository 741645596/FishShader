...
namespace UnityEngine.UI
{
        ...
        ISelectHandler, IDeselectHandler
    {
        ...
        protected ButtonScaleData buttonScaleData;
        ...
        protected virtual void DoStateTransition(SelectionState state, bool instant)
        {
            ...
            if (buttonScaleData != null && buttonScaleData.EnableScale)
            {
                if (state == SelectionState.Pressed)
                {
                    if (!buttonScaleData.isPress)
                    {
                        buttonScaleData.isPress = true;
                        buttonScaleData.OriginScale = gameObject.transform.localScale;
                    }
                    gameObject.transform.localScale = buttonScaleData.OriginScale * buttonScaleData.ClickScale;
                }
                else
                {
                    if (buttonScaleData.isPress)
                    {
                        buttonScaleData.isPress = false;
                        gameObject.transform.localScale = buttonScaleData.OriginScale;
                    }
                }
