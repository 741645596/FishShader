...
namespace UnityEditor.UI
{
    ...
    public class ImageEditor : GraphicEditor
    {
        ...
        protected SerializedProperty MutilLanguage;
        protected SerializedProperty ImagePath;

        bool MutilLanguageState;
        ...
        protected override void OnEnable()
        {
            ...
            MutilLanguage = serializedObject.FindProperty("MutilLanguage");
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.EndFadeGroup();
            NativeSizeButtonGUI();
            EditorGUILayout.PropertyField(MutilLanguage);
            Image com = (Image)target;
            if (MutilLanguageState != com.MutilLanguage)
            {
                MutilLanguageState = com.MutilLanguage;
                var newSprite = m_Sprite.objectReferenceValue as Sprite;
                if (newSprite)
                {
                    string path = UnityEditor.AssetDatabase.GetAssetPath(newSprite);
                    path = path.Replace("Assets/GameData/", "");
                    if (path.StartsWith("LanguageRes"))
                    {
                        if (MutilLanguageState)
                        {
                            com.ImagePath = path;
                        }
                        else
                    ...
                    {
                        com.ImagePath = "";
                        com.MutilLanguage = false;
                        Debug.LogError($"无法设置多语言图片 {path} 请把图片资源放在LanguageRes 对应的语言图集下目录下");
                    }
                }
            }
            if (MutilLanguageState)
