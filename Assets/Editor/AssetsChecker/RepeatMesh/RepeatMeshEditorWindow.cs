...
public class RepeatMeshEditorWindow : AssetCheckEditorWindowBase<RepeatMeshInfo>
{
    ...
    public override void Reload()
    {
        ...
        // 组织多层级列表
        var datas = new List<TableView<RepeatMeshInfo>.TableCellItem>(_showInfos.Count);
        int idIndex = 1;
        for (int i = 0; i < _showInfos.Count; i++)
        {
            ...
            for (int meshIndex = 0; meshIndex<info.repeatDatas.Count; meshIndex++)
            {
                ...
                var item = new TableView<RepeatMeshInfo>.TableCellItem
                {
                    id = idIndex++,                 // id必须唯一
    ...
    public override void TableViewDidShowCell(RepeatMeshInfo info, Rect rect, int rowIndex, TableView<RepeatMeshInfo>.TableCellItem item)
    {
        // 子节点不显示图标
