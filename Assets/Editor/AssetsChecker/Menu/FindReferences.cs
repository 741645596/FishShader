...
namespace EditerUtils
{
    ...
    public class FindReferences
    {
        ...
        [MenuItem("Assets/查找引用该资源的对象", false, 0)]
        private static void Find()
        {
            ...
            //FindRes(filePath, (paths) =>
            //{
            //    foreach (var path in paths)
            //    {
            //        var pp = GetRelativeAssetsPath(path);
            //        Debug.Log(path, AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(pp));
            //    }
            //    Debug.Log($"共找到{paths.Count}个资源引用了{Selection.activeObject.name.ToLower()}");
            //});
        }

        //[MenuItem("Assets/查找引用该资源的C#脚本")]
        static private void FindResInCS()
        {
            // 清除控制台信息
            LogHelper.ClearLogConsole();
        ...
        /// <param name="cb"></param>
        static public void IsResInCS(string filePath, Action<bool> cb)
        {
            GetResPathsInCS(filePath, (paths) =>
            {
                cb(paths.Count != 0);
        ...
        // 检查路径
        static private string _GetSearchPath()
        {
