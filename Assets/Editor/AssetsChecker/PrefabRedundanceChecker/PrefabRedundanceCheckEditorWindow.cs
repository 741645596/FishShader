...
public class PrefabRedundanceCheckEditorWindow : AssetCheckEditorWindowBase<PrefabRedundanceAssetInfo>
{
    ...
    private void _ShowRuleDes()
    {
        const string s_Des = "检查规则：\n" +
            "检测prefab是否引用到了Packages/目录下的资源(Shader、Material等)，这会造成ab冗余和内存重复加载。\n" +
            "备注1：如果是ParticlesUnlit.mat冗余，可以先使用：预制粒子 -> 全部修复 尝试修复一下\n" +
            "备注2：如果是Lit.mat/ParticlesUnlit.mat冗余，在打包机上Library/PackageCache搜索该文件及meta文件然后删除\n" +
            "备注3：如果是Spine这类第三方资源，可以直接在Unity内设置统一的ab包名。(重要提醒：ab名必须在GameAsset有设置过，否则可能会被清空)";
