...
public static class AtlasChecker
{
    ...
    public static AtlasAssetInfo GetAssetInfo(string file)
    {
        ...
        info.isSprite2DFormat = AtlasChecker.IsAllSprite2DFormat(targetFiles);
        info.isAllASTC_4x4 = IsAllASTC4x4(targetFiles);
    ...
    /// <param name="assetPaths"></param>
    /// <returns></returns>
    public static bool IsAllASTC4x4(List<string> assetPaths)
    {
        foreach (var path in assetPaths)
        {
            if (IsASTC4x4OrCloseOverride(path) == false)
            {
                return false;
            }
        }
    ...
    public static bool IsASTC4x4OrCloseOverride(string assetPath)
    {
        ...
        return androidSettings.format == TextureImporterFormat.ASTC_4x4 &&
    ...
    private static List<AtlasAssetInfo> _GetNotExistAtlasDir()
    {
        ...
        foreach (var dir in dirs)
        {
            ...
            info.fileChilds = targetFiles;
            info.isSprite2DFormat = IsAllSprite2DFormat(targetFiles);
            info.isAllASTC_4x4 = IsAllASTC4x4(targetFiles);
    ...
    private static void _InitAtlasFormat(AtlasAssetInfo info)
    {
        ...
        //info.allowRotation = asset.GetPackingSettings().enableRotation;
