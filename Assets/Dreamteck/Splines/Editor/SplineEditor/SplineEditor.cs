namespace Dreamteck.Splines.Editor
{
    ...
    {
        string[] nodeTypeoptions = new string[] { "直线", "曲线", "原地" };
        int[] nodeIntTypeoptions = new int[] { 1, 2, 3 };
        ...
        private void Initialize(Matrix4x4 transformMatrix, SerializedObject splineHolder)
        {
            ...
            //Debug.LogError($"SplineEditor {toolContents.Length}");
        ...
        protected virtual void PointMenu()
        {
            ...
            int selectedIndex = selectedPoints[0];
            for (int i = 0; i < selectedPoints.Count; i++)
            {
            ...
            avgPoint.time = points[selectedPoints[0]].time;
            avgPoint.movetype = points[selectedPoints[0]].movetype;
            avgPoint.playani = points[selectedPoints[0]].playani;
            SplinePoint.Type lastType = avgPoint.type;
            float lastTime = avgPoint.time;
            int lastMoveType = avgPoint.movetype;
            int lastPlayAni = avgPoint.playani;
            ...
            //Debug.Log($"avgPoint.movetype {avgPoint.time} {avgPoint.playani} {avgPoint.movetype}");
            if (selectedIndex > 0)
            {
                avgPoint.time = EditorGUILayout.FloatField("移动时长", avgPoint.time);
                avgPoint.playani = EditorGUILayout.IntField("触发动作", avgPoint.playani);
                avgPoint.movetype = EditorGUILayout.IntPopup("移动类型:", avgPoint.movetype, nodeTypeoptions, nodeIntTypeoptions);
            }
            ...
            for (int i = 0; i < selectedPoints.Count; i++)
            {
                ...
                if (selectedIndex > 0)
                {
                    if (lastTime != avgPoint.time) points[selectedPoints[i]].time = avgPoint.time;
                    if (lastMoveType != avgPoint.movetype) points[selectedPoints[i]].movetype = avgPoint.movetype;
                    if (lastPlayAni != avgPoint.playani) points[selectedPoints[i]].playani = avgPoint.playani;
                }
