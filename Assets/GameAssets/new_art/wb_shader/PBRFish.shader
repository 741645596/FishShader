...
Shader "WB/PBRFish"
{
	Properties
	{
		...
		[FoldoutItem] _RimSpread("RimSpread[边缘扩展参数]", Range( 0 , 10)) = 1
		...
		[FoldoutItem] _HSVHue("色调", Range(0, 10)) = 0.5
		[FoldoutItem] _HSVSat("饱和度", Range(0, 10)) = 1
		[FoldoutItem] _HSVValue("亮度", Range(0, 10)) = 1
		...
		[FoldoutItem] [ToggleUI]_IsHurt("挨打状态", Float) = 0
		[FoldoutItem] _HurtColor("挨打颜色", Color) = (1, 0, 0, 1)
		[FoldoutItem] _HurtMultiple("HurtMultiple程序控制", Float) = 1
		[FoldoutItem] _HurtMultipleCust("HurtMultiple美术控制", Float) = 1
		...
		[FoldoutItem][NoScaleOffset] _StreamerNoise("StreamerNoise", 2D) = "black" {}
		[FoldoutItem][NoScaleOffset] _StreamerMask("StreamerMask[流光mask]", 2D) = "black" {}
		[FoldoutItem][NoScaleOffset] _StreamerTex("StreamerTex[流光纹理]", 2D) = "black" {}
		...
		[FoldoutItem] _StreamerScrollX("StreamerScrollX[流光方向]", Range(0, 10)) = 1
		[FoldoutItem] _StreamerScrollY("StreamerScrollY[流光方向]", Range(0, 10)) = 1
...
//#endif
			...
			half4 frag(VertexOutput IN) : SV_Target
			{
				...
				float4 Albedo = tex2D( _BaseMap, uv_BaseMap ).rgba * _BaseColor.rgba;
				...
				float3 Emission = rimColor + emission.rgb;
				...
				half4 fishColor = color;
				fishColor.rgb = hurt(fishColor.rgb, inputData.normalWS, inputData.viewDirectionWS);
				color.rgb = fishColor.rgb;
				return color;
			}
