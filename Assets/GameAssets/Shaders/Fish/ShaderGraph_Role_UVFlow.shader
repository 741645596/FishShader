Shader "Fish/ShaderGraph_Role_UVFlow" {
    ...
    SubShader {
        ...
        Pass {
            ...
            float Unity_FresnelEffect_half(half3 Normal, half3 ViewDir, half Power, out half Out)
            {
                // return pow(min((1.0 - saturate(dot(normalize(Normal), normalize(ViewDir)))), 0.01), Power);
			...
			half4 fragUVFlowColor(Varyings fInput)
			{
                ...
                mainTexColor.xyz = GammaToLinearSpace3(mainTexColor.xyz);
    ...
    output.screenPosition = ComputeScreenPos(output.positionCS, _ProjectionParams.x);
            ...
            half4 frag(Varyings input) : SV_TARGET
            {
            	...
            	//half3 baseColor = hurt(surfaceDescription.BaseColor, input.normalWS, input.viewDirectionWS);
                half3 baseColor = HitRed(surfaceDescription.BaseColor, float3(0, 0, 0), input.normalWS, input.viewDirectionWS);
            	
				baseColor += uvColor.rgb;
				return LocalGammaToLinear(half4(baseColor, alpha * _FishAlpha));
            }
            #pragma vertex vert
