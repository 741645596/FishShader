Shader "Fish/FishShaderGraph_More" {
    Properties {
        ...
        _AlphaClipThreshold("透明度裁剪阈值", Float) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            SurfaceDescription SurfaceDescriptionFunction(SurfaceDescriptionInputs IN)
            {
                ...
                texMain.xyz = GammaToLinearSpace3(texMain.xyz);
                ...
                float3 _FishColor;
                color_float(texMainBld, _MainColor, 1, _AlbedoIntensity,
                            (float4(clrMat, 1.0)),
                            (rimClr.xyz),
                            _FishColor);
                //_FishColor = hurt(_FishColor, IN.WorldSpaceNormal, IN.viewDirectionWS);
                _FishColor = HitRed(_FishColor, float3(0, 0, 0), IN.WorldSpaceNormal, IN.viewDirectionWS);


                float4 texTransparency = SAMPLE_TEXTURE2D(_TexTransparency, sampler_TexTransparency,
                                                          IN.uv0.xy);

                float AlphaClipThreshold = 1-_AlphaClipThreshold;
                
                SurfaceDescription surface = (SurfaceDescription)0;
                surface.BaseColor = _FishColor;
                surface.Alpha = texTransparency.r;
                surface.AlphaClipThreshold = AlphaClipThreshold;
                return surface;
            }
    ...
    output.screenPosition = ComputeScreenPos(output.positionCS, _ProjectionParams.x);
                ...
                output.viewDirectionWS.xyz = _WorldSpaceCameraPos - positionWS;
    ...
    surfaceDescription.BaseColor *= surfaceDescription.Alpha;
                ...
                return LocalGammaToLinear(half4(surfaceDescription.BaseColor, alpha));
            }
            #pragma vertex vert
